...
#Grafico
```{r}
programas<-c("Python","R", "SQL","SAS","STATA")
calificacion<-c(2,5,3,4,1)
p<-data.frame(programas,calificacion)
ggplot2::ggplot(p, aes(x=programas, y=calificacion, fill=calificacion))+geom_bar(stat = "identity", width = 0.85)+ coord_flip()
```